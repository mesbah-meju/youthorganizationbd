...
class LoginController extends Controller
{
    ...
    */
    ...
    public function handleProviderCallback(Request $request, $provider)
    {
        ...
        //check if provider_id exist
        $existingUserByProviderId = User::where('provider_id', $user->id)->where('status', 1)->first();
        ...
        if ($existingUserByProviderId) {
            ...
            //check if email exist
            $existingUser = User::where('email', '!=', null)->where('email', $user->email)->where('status', 1)->first();
    ...
    public function authenticated()
    {
        if (auth()->user()->user_type == 'developer' || auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'directorate') {
            flash(translate("WELCOME TO YOUTH ORG DB"))->success();

            return redirect()->route('admin.dashboard');
        } elseif (auth()->user()->user_type == 'organization') {
            $organization_id = Organization::where('user_id', auth()->user()->id)->value('id');
    ...
    {
        if (auth()->user() != null && (auth()->user()->user_type == 'developer' || auth()->user()->user_type == 'admin' || auth()->user()->user_type == 'directorate')) {
