...
class BusinessSettingsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:schedule_settings'])->only('schedule_settings');
    ...
    {
        $schedule_duration = BusinessSetting::where('type', 'schedule_duration')->value('value');
        return view('backend.setup_configurations.schedule_settings', compact('schedule_duration'));
    }
     ...
     */
     ...
     public function schedule_settings_update(Request $request)
    {
        foreach ($request->types as $key => $type) {
            $this->overWriteEnvFile($type, $request[$type]);
        }
        $business_settings = BusinessSetting::where('type', 'schedule_settings')->first();
        $schedule_duration = BusinessSetting::where('type', 'schedule_duration')->first();

        if (!$business_settings) {
            $business_settings = new BusinessSetting();
            $business_settings->type = 'schedule_settings';
        }

        $business_settings->value = $request->has('schedule_settings') ? 1 : 0;
        $business_settings->save();

        if (!$schedule_duration) {
            $schedule_duration = new BusinessSetting();
            $schedule_duration->type = 'schedule_duration';
        }
        $schedule_duration->value = $request->has('schedule_duration') ? $request->schedule_duration : 0;
        $schedule_duration->save();
