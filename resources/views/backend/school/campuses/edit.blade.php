<form class="form-horizontal"  action="{{ route('campuses.update', $campus->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="w-100">
        <div class="form-row">
            {{-- <div class="form-group col-md-6">
                ...
                <div class="">
                    <select name="school_id" id="school_id" class="form-control aiz-selectpicker" onchange="schoolWiseClassForScheduleCreate(this.value)">
                        ...
                        <option value="{{ $school->id }}"@selected($school->id == $campus->school_id)>{{ $school->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div> --}}
            ...
            <div class="form-group col-md-6">
                ...
                <input type="text" class="form-control" name="campus_name" id="campus_name" value="{{ $campus->name }}" placeholder="{{ translate('Campus Name') }}" required>
            </div>

            <div class="form-group col-md-12">
                <label class="from-label" for="address">{{ translate('Address') }}</label>
                <textarea name="address" id="address" class="form-control" placeholder="{{ translate('Address') }}">{{$campus->address}}</textarea>
            </div>
