...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{ translate('Deputy Directors List') }}</h1>
        </div>
    </div>
</div>
<div class="card">
    <form id="sort_debit_vouchers" action="{{ route('deputy_directors_list_report.index') }}" method="GET">
        <div class="card-body">
            <div class="row">
                <div class="col-sm-4">
    ...
    <div class="card-body printArea">
        <div class="row pb-3 align-items-center">
            <div class="col-md-3">
                ...
                <img src="{{ uploaded_asset(get_setting('system_logo_white')) }}" class="brand-icon" alt="{{ get_setting('site_name') }}" style="width: 60px;">
                @else
                <img src="{{ static_asset('assets/img/logo.png') }}" class="brand-icon" alt="{{ get_setting('site_name') }}" style="width: 60px;">
                @endif
            </div>
        ...
        <div class="row pb-3 voucher-center align-items-center">
            <div class="col-md-12 text-center">
                <strong><u class="pt-4">{{ translate('Deputy Directors List Report') }}</u></strong>
            </div>
        </div>
        ...
        <div class="table-responsive">
            <table class="datatable table table-striped table-hover" cellpadding="6" cellspacing="1">
                ...
                <tbody>
                    @foreach($deputyDirectors as $director)
                    <tr>
                        <td>1</td>
                        <td>{{ $director->name }}</td>
                        ...
                        <td>
                            @if($director->divisionN)
                                {{ $director->divisionN->name }}
                            @else
                                'N/A'
                            @endif
                        </td>
                        <td>
                            @if($director->districtN)
                                {{ $director->districtN->name }}
                            @else
                                'N/A'
                            @endif
                        </td>
                        <td>
                            @if($director->upazilaN)
                                {{ $director->upazilaN->name }}
                            @else
                                'N/A'
                            @endif
                        </td>
                        <td>{{ $director->gov_id }}</td>
