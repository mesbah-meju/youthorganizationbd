                ...
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" placeholder="{{translate('Name')}}" id="name" name="name" class="form-control" value="{{ old('name', $director->name ?? '') }}" required>
                            @error('name')
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" placeholder="{{translate('Email')}}" id="email" name="email" class="form-control" value="{{ old('email', $director->email ?? '') }}" required>
                            @error('email')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="form-group col-md-4">
                            ...
                            <input type="text" placeholder="{{translate('Phone')}}" id="phone" name="phone" class="form-control" value="{{ old('phone', $director->phone ?? '') }}" required>
                            @error('phone')
                                <small class="form-text text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        <div class="form-group col-md-4">
                            <label class="from-label" for="password">{{translate('Password')}}</label>
                        ...
                        <div class="form-group col-md-4">
                            <label class="from-label" for="name">{{translate('Designation')}} <span class="text-danger">*</span></label>
                            <select name="role_id" required class="form-control aiz-selectpicker">
                                @foreach($roles as $role)
                                    <option value="{{ $role->id }}" {{ old('role_id', $director->userrole->role_id ?? '') == $role->id ? 'selected' : '' }}>{{$role->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group col-md-4">
                            ...
                            <select name="division_id" id="division_id" class="form-control aiz-selectpicker" onchange="divisionWiseDistrict(this.value)">
                                ...
                                <option value="{{ $division->id }}" {{ old('division_id', $director->division ?? '') == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                                @endforeach
                            </select>
                        </div>

                        @if($director->role_id == 5)
                        <div class="form-group col-md-4" style="" id="district_div">
                            <label class="from-label" for="district_id">{{ translate('District') }} <span class="text-danger">*</span></label>
                            <select name="district_id" id="district_id" class="form-control aiz-selectpicker" onchange="districtWiseUpazila(this.value)" required>
                                <option value="">Select district</option>
                                @foreach($districts as $district)
                                <option value="{{ $district->id }}" {{ old('district_id', $director->district ?? '') == $district->id ? 'selected' : '' }}>{{ $district->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @else
                        ...
                        @if($director->role_id == 5)
                        <div class="form-group col-md-4" style="" id="upazila_div">
                            ...
                            <select name="upazila_id" id="upazila_id" class="form-control aiz-selectpicker" required>
                                ...
                                <option value="{{ $upazila->id }}" {{ old('upazila_id', $director->upazila ?? '') == $upazila->id ? 'selected' : '' }}>{{ $upazila->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @else
