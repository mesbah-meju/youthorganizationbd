...
<div class="row">
    <div class="col-lg-6 mx-auto">
        <div class="card">
            ...
            <form action="{{ route('directors.update', $user->id) }}" method="POST">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <input type="text" placeholder="{{translate('Phone')}}" id="mobile" name="mobile" value="{{ $user->phone }}" class="form-control" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            <select name="role_id" required class="form-control aiz-selectpicker">
                                @foreach($roles as $role)
                                    <option value="{{$role->id}}" @php if($user->role_id == $role->id) echo "selected"; @endphp >{{$role->name}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
