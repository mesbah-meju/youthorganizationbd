        ...
        <form id="sort_schools" action="" method="GET">
            <div class="card-header row gutters-5">
                <div class="col d-flex justify-content-between align-items-center">
                    <h5 class="mb-0 h6">{{ translate('Rejected Organizations') }}</h5>
                </div>
            </div>

            <div class="card-body">
                <table class="table mb-0" id="approved-organization-table">
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th>{{ translate('Rejectd By') }}</th>
                            <th>{{ translate('Rejectd Date') }}</th>
                        ...
                        @if ($organizations->isNotEmpty())
                            @foreach ($organizations as $key => $organization)
                                <tr>
                                    ...
                                    <td>
                                        @if($organization->rejectdBy)
                                        {{ $organization->rejectdBy->name }}
                                        @else
                                        -
                                        @endif
                                    </td>
                                    <td>
                                        {{ date('F d, Y', strtotime($organization->rejected_at)) }}
                                    </td>
                                    <td>
...
@section('script')
    <script>
        $(document).ready(function() {
            $('#approved-organization-table').DataTable({
                paging: true,
                searching: true,
                ordering: true,
                info: true,
